...
export const updateOrderStatus = async (orderId, body = {}, currentUser) => {
  try {
    ...
    if (status === 'da_vao_khung') {
      ...
      const isCuttingNotRequired = frameCuttingStatus === 'khong_cat_khung';
      if (
        printingStatus !== 'san_xuat_da_nhan_tranh' ||
        (!isCuttingNotRequired && frameCuttingStatus !== 'da_cat_khung')
      ) {
        return buildServiceResponse(400, {
          success: false,
          message: 'Chỉ có thể đánh dấu "Đã vào khung" khi sản xuất đã nhận cả tranh và khung'
        });
      }
    }
    ...
    if (role === 'in') {
      ...
      if (allowedStatuses.includes(order.printingStatus)) {
        ...
        if (!hasTranhKhung && order.status === 'dang_xu_ly' && order.canTransitionTo('cho_dong_goi')) {
          ...
          // Nếu đã cắt khung, chuyển sang chờ sản xuất
          order.status = 'cho_san_xuat';
          await order.addStatusHistory('cho_san_xuat', user._id, 'Đã in và cắt khung xong');
        }
      } else {
        return buildServiceResponse(400, {
          success: false,
  ...
  try {
    ...
    if (type === 'tranh') {
      ...
      if (order.frameCuttingStatus === 'da_cat_khung') {
        ...
        return buildServiceResponse(400, {
          success: false,
          message: 'Chỉ có thể nhận khung khi đã cắt khung xong'
        });
      }
    }
