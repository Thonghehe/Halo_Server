...
function Orders() {
  ...
  // Tính pageLoading dựa trên ordersLoading và ordersResp
  // Chỉ hiển thị pageLoading khi đang loading lần đầu (chưa có data)
  const pageLoading = useMemo(() => {
    if (ordersLoading) {
      // Chỉ hiển thị pageLoading nếu chưa có data gì cả
      const hasData = ordersResp?.data !== undefined || ordersResp?.pagination !== undefined;
      return !hasData;
    }
    // Khi đã load xong, không hiển thị pageLoading
    return false;
  }, [ordersLoading, ordersResp]);
  ...
  useEffect(() => {
    // Nếu đang loading
    if (ordersLoading) {
      // Chỉ set tableLoading khi đang refetch (đã có data từ cache)
      const hasData = ordersResp?.data !== undefined || ordersResp?.pagination !== undefined;
      if (hasData) {
    ...
    // Khi đã load xong, reset tableLoading
  ...
  // Đồng bộ pendingQuery với filters khi component mount (tương tự admin-panel)
  useEffect(() => {
    // Khi component mount, đồng bộ pendingQuery với filters để tránh refetch không cần thiết
