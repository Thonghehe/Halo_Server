...
function Orders() {
  ...
  useEffect(() => {
    // Khi đã load xong (ordersLoading = false), luôn reset loading state TRƯỚC
    if (!ordersLoading) {
    ...
    // Nếu đang loading
    if (ordersLoading) {
      // Chỉ set pageLoading = true nếu chưa có data gì cả (lần đầu load)
      // Nếu đã có data (từ cache), chỉ set tableLoading để hiển thị loading trên table
      const hasData = ordersResp?.data !== undefined || ordersResp?.pagination !== undefined;
      if (!hasData) {
        setPageLoading(true);
      }
  ...
  // Đảm bảo pageLoading được reset ngay khi component mount nếu đã có data
  useEffect(() => {
    // Khi component mount, nếu React Query đã có data (từ cache) và không đang loading, reset pageLoading ngay
    if (!ordersLoading && (ordersResp?.data !== undefined || ordersResp?.pagination !== undefined)) {
      setPageLoading(false);
      setTableLoading(false);
    }
