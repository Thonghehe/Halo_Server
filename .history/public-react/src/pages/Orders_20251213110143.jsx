...
const buildQueryString = (q = {}) => {
  ...
  if (q.paintingType) params.append('paintingType', q.paintingType);
...
function Orders() {
  ...
  // Debounced search để tránh refetch liên tục khi gõ
  const [debouncedSearch, setDebouncedSearch] = useState(pendingQuery.search);
  
  useEffect(() => {
    // Nếu xóa search (về rỗng), update ngay lập tức, không debounce
    if (pendingQuery.search === '') {
      setDebouncedSearch('');
      return;
    }
    // Nếu đang gõ, debounce 400ms
    const handler = setTimeout(() => {
      setDebouncedSearch(pendingQuery.search);
    }, 400);
    return () => clearTimeout(handler);
  }, [pendingQuery.search]);

  // Load orders với filters từ pendingQuery (dùng debouncedSearch cho search field)
  const queryFilters = useMemo(() => {
    const q = { ...pendingQuery, search: debouncedSearch };
    q.page = page;
    q.limit = limit;
    ...
    return q;
  }, [pendingQuery, debouncedSearch, page, limit, sortOrder]);
  ...
  const { data: ordersResp = {}, isLoading: ordersLoading, refetch: refetchOrders } = useOrders(queryFilters);
  ...
  useEffect(() => {
    ...
    const urlFilters = {
      ...
      paintingType: urlParams.get('paintingType') || ''
    };
  ...
  // Update URL when filters, page, limit, or sortOrder change
  useEffect(() => {
    // Skip URL update during initial load from URL to avoid infinite loop
  ...
  // Auto-load table when search text changes (debounced) - React Query sẽ tự động refetch khi queryFilters thay đổi
  useEffect(() => {
  ...
  // Auto-load table immediately when filters change - React Query sẽ tự động refetch khi queryFilters thay đổi
  useEffect(() => {
    // Skip during initial load from URL to avoid unnecessary refetch
    if (isInitialLoadFromURL.current) {
      return;
    }
    setPage(1);
        ...
        <div className="card mb-4">
          <div className="card-body">
            ...
            <div className="col-12 col-md-2">
              ...
              <select
                className="form-select"
                value={pendingQuery.paintingType}
                onChange={(e) => setPendingQuery({ ...pendingQuery, paintingType: e.target.value })}
              >
