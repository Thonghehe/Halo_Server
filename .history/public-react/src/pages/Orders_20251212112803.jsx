...
function Orders() {
  ...
  // Khởi tạo pageLoading dựa trên ordersLoading từ React Query
  // Nếu React Query đã có cache, ordersLoading sẽ là false ngay, nên pageLoading cũng nên là false
  ...
  useEffect(() => {
    // Nếu đang loading, set loading state
    if (ordersLoading) {
      // Nếu chưa có data, set cả pageLoading và tableLoading
      if (!ordersResp?.data) {
        setPageLoading(true);
        setTableLoading(true);
      } else {
        // Nếu đã có data (từ cache) nhưng đang refetch, chỉ set tableLoading
        setTableLoading(true);
      }
      return;
    }

    // Khi đã load xong, xử lý data
    ...
    // Luôn set loading = false khi đã load xong
    setTableLoading(false);
    setPageLoading(false);
  }, [ordersResp, ordersLoading, sortOrdersData, page, limit]);

  // Đảm bảo pageLoading được reset khi component mount lại (tương tự admin-panel)
  // Kiểm tra ngay khi ordersResp có data hoặc ordersLoading thay đổi
  useEffect(() => {
    // Nếu React Query đã có data (từ cache) và không đang loading, reset pageLoading ngay
    if (!ordersLoading) {
      // Nếu đã có data hoặc đã có pagination, nghĩa là đã load xong
      if (ordersResp?.data !== undefined || ordersResp?.pagination !== undefined) {
        setPageLoading(false);
      }
    }
  }, [ordersResp, ordersLoading]);
