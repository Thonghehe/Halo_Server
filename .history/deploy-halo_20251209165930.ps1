...
$remoteCmd = "cd $serverPath && git pull origin $branch && docker compose -f docker-compose.ssl.yml stop backend admin-panel public && docker compose -f docker-compose.ssl.yml build --no-cache backend admin-panel public && docker compose -f docker-compose.ssl.yml up -d backend admin-panel public && docker exec halo-nginx-ssl nginx -s reload"
