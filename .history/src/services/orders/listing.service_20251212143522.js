...
export const getOrders = async (query = {}) => {
  try {
    ...
    // Lọc theo loại tranh (tranh_dan | tranh_khung | tranh_tron)
    if (paintingType) {
      const type = paintingType.trim();
      if (type.length > 0) {
        const paintingOrderIds = await Painting.find({ type }).distinct('orderId');
        if (paintingOrderIds.length === 0) {
          return buildServiceResponse(200, { success: true, message: 'Lấy danh sách đơn hàng thành công', data: [], pagination: { page: Number(page), limit: Number(limit), total: 0, pages: 0 } });
        }
        filter._id = { $in: paintingOrderIds.map((id) => new mongoose.Types.ObjectId(id)) };
      }
    }
