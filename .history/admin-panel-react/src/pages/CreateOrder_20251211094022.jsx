...
const [mentionUserModal, setMentionUserModal] = useState({ show: false, user: null });
  ...
  // Đảm bảo paintingTextareaRefs.current luôn là object
  if (!paintingTextareaRefs.current || typeof paintingTextareaRefs.current !== 'object') {
  ...
  const handleUpdateProfitSharingPercentage = (index, percentage) => {
    // Parse percentage, cho phép số thập phân
    let numericValue = 0;
    if (percentage !== undefined && percentage !== null && percentage !== '') {
      const cleaned = percentage.toString().replace(/[^\d.,]/g, '').replace(',', '.');
  ...
  useEffect(() => {
    if (formData.profitSharing && formData.profitSharing.length > 0 && formData.paintingPrice !== undefined) {
      const paintingPrice = formData.paintingPrice || 0;
      setFormData(prev => {
        // Chỉ cập nhật nếu có thay đổi thực sự
        const updated = prev.profitSharing.map(item => {
          const newAmount = Math.round((paintingPrice * (item.percentage || 0)) / 100);
          if (item.amount === newAmount) {
            return item; // Không thay đổi nếu amount giống nhau
          }
          return {
            ...item,
            amount: newAmount
        ...
        const hasChanges = updated.some((item, index) => item.amount !== prev.profitSharing[index]?.amount);
        if (!hasChanges) {
          return prev; // Không cần cập nhật
        }
  ...
  // Upload ảnh/file cọc (nhiều file)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // For shopee orders, set customer info using orderCodeCustom as shopee order number
      if (formData.orderType === 'shopee') {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="card mb-4">
          <div className="card-body">
            ...
            {formData.orderType === 'shopee' ? null : (
              <>
                <div className="row mb-3">
                  <div className="col-md-6">
        ...
        {formData.orderType !== 'shopee' && (
          <div className="card mb-4">
            <div className="card-body">
              <div className="d-flex justify-content-between align-items-center mb-3">
                <h5 className="form-section-title mb-0">Phân chia tỉ lệ (Nếu có)</h5>
              ...
              {!formData.profitSharing || formData.profitSharing.length === 0 ? (
                <p className="text-muted mb-0">Chưa có người nào được thêm vào danh sách phân chia tỉ lệ (%)</p>
              ) : (
                    ...
                    <tbody>
                      {formData.profitSharing.map((item, index) => {
                        ...
                        const user = mentionableUsers?.find(u => {
                          ...
                          return u._id.toString() === itemUserId?.toString();
                        });
                        return (
                          <tr key={index}>
                            <td>
                              {user ? (user.fullName || user.email || 'Người dùng') : 'Người dùng'}
                            </td>
                            <td>
        ...
        {/* Hình thức gửi đơn - chỉ hiển thị với đơn hàng không phải Shopee */}
        {formData.orderType !== 'shopee' && (
          <div className="card mb-4">
            <div className="card-body">
              <div className="d-flex justify-content-between align-items-center mb-3">
                ...
                <div className="form-check form-switch">
                  <input
                    className="form-check-input"
                    type="checkbox"
                    id="enableShippingInfo"
                    checked={shippingInfoEnabled}
                    onChange={(e) => handleToggleShippingInfo(e.target.checked)}
                  />
                  <label className="form-check-label" htmlFor="enableShippingInfo">
                    Nhập thông tin gửi đi
                  </label>
                </div>
              </div>
              {!shippingInfoEnabled ? (
                <p className="text-muted mb-0">Bỏ trống để kế toán điều đơn ghi thông tin giao hàng sau.</p>
              ) : (
                <>
                  <div className="row g-3">
                    <div className="col-md-6">
