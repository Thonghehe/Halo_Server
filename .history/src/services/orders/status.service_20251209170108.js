...
export const completeOrder = async (orderId, payload = {}, currentUser) => {
  try {
    ...
    if (role === 'in') {
      ...
      // Chặn gửi đi nếu đang có bản nháp tiền chờ admin duyệt
      if (order.pendingMoneyDraft && order.pendingMoneyDraft.status === 'pending') {
        return buildServiceResponse(400, {
          success: false,
          message: 'Đơn hàng đang chờ admin duyệt thay đổi, không thể gửi đi'
        });
      }
      ...
      if (order.status === 'da_gui_di') {
        ...
        // Cập nhật tiền thực nhận nếu truyền vào
        const normalizedActualReceived = Number(actualReceivedAmount);
        if (!Number.isNaN(normalizedActualReceived) && normalizedActualReceived >= 0) {
          order.actualReceivedAmount = normalizedActualReceived;
        }
        ...
        const moneyNote =
          !Number.isNaN(normalizedActualReceived) && normalizedActualReceived >= 0
            ? ` - Thực nhận: ${normalizedActualReceived.toLocaleString('vi-VN')}đ`
            : '';
        statusNote = `${displayName} đã hoàn thành đơn hàng${moneyNote}`;
