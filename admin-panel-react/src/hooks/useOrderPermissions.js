...
export function useOrderPermissions(order, user) {
  // Helper: Kiểm tra đơn hàng có tranh khung không
  ...
  const canCompleteOrder = useMemo(() => {
    ...
    if (hasKeToanTaiChinhRole) {
      ...
      if (currentStatus === 'da_gui_di') {
        return { canComplete: true, role: 'keToanTaiChinh', label: 'Đã hoàn thành đơn hàng' };
      }
    }
  ...
  const canFrameOrder = useMemo(() => {
    ...
    return (
      ...
      (frameCuttingStatus === 'da_cat_khung' || frameCuttingStatus === 'khong_cat_khung')
    );
  }, [user, order]);
